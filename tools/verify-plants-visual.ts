#!/usr/bin/env tsx
/**
 * Strict visual verification for library gallery plants (newPlants row).
 * Requires dev server on :5173.
 */
import { mkdirSync } from 'node:fs';
import sharp from 'sharp';
import { NEW_PLANT_VERIFY_GATES } from '../src/vegetation/library/gallery';
...
async function main(): Promise<void> {
  ...
  console.log(`[verify-plants] ${url}`);
  await page.goto(url, { waitUntil: 'domcontentloaded', timeout: 300000 });
  await waitForLaasReady(page, { timeoutMs: 600000 });
  await laasSettle(page, 48);

  const screenshotPath = 'shots/profiles/new-plants-gate.png';
  ...
  for (const gate of NEW_PLANT_VERIFY_GATES) {
    let matchCount = 0;
    ...
    console.log(
      `[verify-plants] ${gate.name}: ${matchCount}px (min ${gate.minPxCount}) → ${passed ? 'PASS' : 'FAIL'}`,
    );
  ...
  console.log('[verify-plants] all gates passed');
}
