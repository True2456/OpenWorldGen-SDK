#!/usr/bin/env tsx
/**
 * Visual verification for Asia continent trees gallery row.
 * Requires dev server on :5173.
 */
import { mkdirSync } from 'node:fs';
import sharp from 'sharp';
import { ASIA_TREE_VERIFY_GATES } from '../src/vegetation/library/gallery';
...
async function main(): Promise<void> {
  ...
  const url = laasUrl({
    ...
    preset: 'high',
    freeze: true,
    extra: { row: 'asiaTrees', lite: '1' },
  });

  console.log(`[verify-asia-trees] ${url}`);
  await page.goto(url, { waitUntil: 'domcontentloaded', timeout: 300000 });
  await waitForLaasReady(page, { timeoutMs: 600000 });
  await laasSettle(page, 48);

  const screenshotPath = 'shots/profiles/asia-trees-gate.png';
  ...
  for (const gate of ASIA_TREE_VERIFY_GATES) {
    let matchCount = 0;
    ...
    console.log(
      `[verify-asia-trees] ${gate.name}: ${matchCount}px (min ${gate.minPxCount}) → ${passed ? 'PASS' : 'FAIL'}`,
    );
  ...
  console.log('[verify-asia-trees] all gates passed');
}
