#!/usr/bin/env tsx
/**
 * Visual verification for Africa continent trees gallery row.
 * Requires dev server on :5173.
 */
import { mkdirSync } from 'node:fs';
import sharp from 'sharp';
import { AFRICA_TREE_VERIFY_GATES } from '../src/vegetation/library/gallery';
...
async function main(): Promise<void> {
  ...
  const url = laasUrl({
    ...
    preset: 'high',
    freeze: true,
    extra: { row: 'africaTrees', lite: '1' },
  });

  console.log(`[verify-africa-trees] ${url}`);
  await page.goto(url, { waitUntil: 'domcontentloaded', timeout: 300000 });
  await waitForLaasReady(page, { timeoutMs: 600000 });
  await laasSettle(page, 48);

  const screenshotPath = 'shots/profiles/africa-trees-gate.png';
  ...
  for (const gate of AFRICA_TREE_VERIFY_GATES) {
    let matchCount = 0;
    ...
    console.log(
      `[verify-africa-trees] ${gate.name}: ${matchCount}px (min ${gate.minPxCount}) → ${passed ? 'PASS' : 'FAIL'}`,
    );
  ...
  console.log('[verify-africa-trees] all gates passed');
}
